...
export const mqttClient = mqtt.connect('mqtt://164.92.96.240:1883', {
  ...
  // 'client_ + {랜덤 문자열}' 형태로 생성 -> 실행될 때마다 다른 MQTT 클라이언트로 인식
});

// 사용자별 타이머 정보를 저장하는 Map
export const timers = new Map<string, TimerInfo>(); 
// 'userId → 타이머 정보' 구조로 저장됨
// ex. 
//   "3" -> {
...
function startTimer(userId: string, groupId: string, timestamp: Date) {
  if (timers.has(userId)) {
    ...
    if (timerInfo.isRunning) return; // 이미 타이머가 있고 실행 중이면 리턴(바로 종료) 
    
    // 타이머가 없으면 새로 setInterval을 만듦
    timerInfo.isRunning = true;
    timerInfo.lastTimestamp = timestamp;
  } else {
    // 특정 사용자에 대한 타이머 정보를 timers Map에 등록 후, 1초마다 WebSocket으로 타이머 상태 전송
    timers.set(userId, {
      lastTimestamp: timestamp, // 타이머가 시작된 기준 시각 저장
      interval: setInterval(async () => { // setInterval: 일정 시간마다 코드 반복 실행
        const timerInfo = timers.get(userId); // 현재 타이머 정보에 접근
        if (!timerInfo || !timerInfo.isRunning) return; // 타이머 정보가 없거나 실행 중이 아니면 종료
